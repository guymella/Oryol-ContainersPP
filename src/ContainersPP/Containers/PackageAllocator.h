...
class PackageAllocator: public iProfile {
public:
    /// default constructor
    PackageAllocator(const char* AllocationFolder);
    /// Copy constructor
    //Allocator(const Allocator& rhs) { buffers = rhs.buffers; };
    /// move constructor
    //Allocator(Allocator&& rhs) { buffers = std::move(rhs.buffers); };
    /// destructor
    ~PackageAllocator() { Oryol::Memory::Free(folderName); };

    virtual iCatalogue& operator[](uint64_t index) override {
        Load(index);
        FileCatalogue* &block = catalogues.GetOrAdd(index);
        if (!block)
            new (block) FileCatalogue(folderName, index);
        return *block;
    };
    virtual const iCatalogue& operator[](uint64_t index) const override {
        const auto block = catalogues.Find(index);
        if (!block || !(*block))
            return NULLOBJECT;
        return *(*block);            
    };

    CatalogueBase* Get(uint64_t index);
    const CatalogueBase* Get(uint64_t index) const;

    virtual uint64_t New() override;
    //virtual uint64_t New(uint64_t newSize);

    bool Load(uint64_t BlockID);
    bool Save(uint64_t BlockID);
    bool UnLoad(uint64_t BlockID);
    virtual bool Delete(uint64_t BlockID) override;

    /// get number of buffers
    virtual uint64_t Count() const { return 0; }; //TODO:: count files in folder
   /* /// return true if empty
    bool Empty() const { return !Count(); };*/
private:
    char* folderName = 0;
    uint64_t NextID(uint64_t lastID = 0) const;
    TypeTrie<FileCatalogue*> catalogues;
};

inline PackageAllocator::PackageAllocator(const char* AllocationFolder)
{
...
inline CatalogueBase* PackageAllocator::Get(uint64_t index)
{
    auto block = catalogues.Find(index);
    if (!block)
        return nullptr;
    return *block;
}

inline const CatalogueBase* PackageAllocator::Get(uint64_t index) const
{
    const auto block = catalogues.Find(index);
    if (!block)
        return nullptr;
...
uint64_t ContainersPP::PackageAllocator::New()
{
    uint64_t id = NextID();
    FileCatalogue* blockPtr = catalogues.GetOrAdd(id);
    new (blockPtr) FileCatalogue(folderName,id,this);
    return id;
}

inline bool PackageAllocator::Load(uint64_t BlockID)
{
    if (catalogues.Contains(BlockID))
        return true;

    FileCatalogue* blockPtr = catalogues.GetOrAdd(BlockID);
    new (blockPtr) FileBlock(folderName, BlockID);

    return true;
}

inline bool PackageAllocator::Save(uint64_t BlockID)
{
    if (catalogues.Contains(BlockID))
        return catalogues.GetOrAdd(BlockID)->Save();

    return false;
}

inline bool PackageAllocator::UnLoad(uint64_t BlockID)
{
    if (catalogues.Contains(BlockID)) {
        FileCatalogue* blockPtr = catalogues.GetOrAdd(BlockID);
        if (blockPtr)
            delete (blockPtr);
        catalogues.Erase(BlockID);
        return true;
    }
    return false;   
}

inline bool PackageAllocator::Delete(uint64_t BlockID)
{
    if (catalogues.Contains(BlockID)) {
        FileCatalogue* blockPtr = catalogues.GetOrAdd(BlockID);
        if (blockPtr) {
...
inline uint64_t PackageAllocator::NextID(uint64_t lastID) const
{
